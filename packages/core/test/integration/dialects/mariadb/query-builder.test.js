  ...
  describe('[MARIADB] QueryBuilder', () => {
    ...
    describe('Basic QueryBuilder functionality', () => {
      ...
      it('should generate SELECT query with WHERE clause', () => {
        ...
        expect(query).to.equal('SELECT * FROM `users` AS `User` WHERE `User`.`active` = true;');
      });
      ...
      it('should generate SELECT query with multiple WHERE conditions', () => {
        ...
        expect(query).to.equal(
          'SELECT * FROM `users` AS `User` WHERE `User`.`active` = true AND `User`.`age` = 25;',
        );
      });
      ...
      it('should generate complete SELECT query with attributes and WHERE', () => {
        const query = User.select()
          ...
          .getQuery();
        expect(query).to.equal(
          'SELECT `name`, `email` FROM `users` AS `User` WHERE `User`.`active` = true;',
        );
      });
    });
    ...
    describe('Functional/Immutable behavior', () => {
      ...
      it('should not mutate original builder when chaining', () => {
        ...
        expect(whereQuery).to.equal(
          'SELECT * FROM `users` AS `User` WHERE `User`.`active` = true;',
        );
      });
    ...
    describe('MariaDB-specific features', () => {
      it('should handle MariaDB operators correctly', () => {
        const query = User.select()
          .where({
            name: { [Op.like]: '%john%' },
      ...
      it('should handle array operations', () => {
        ...
        expect(query).to.include("'John'");
        expect(query).to.include("'Jane'");
        expect(query).to.include("'Bob'");
      });

      it('should quote identifiers properly for MariaDB', () => {
        const query = User.select()
          .attributes(['name', 'email'])
          .where({ active: true })
          .getQuery();

        // MariaDB uses backticks for identifiers (like MySQL)
